#s is a callback function to be used with training of Keras models.
...
#        self.model2 = clone_model(self.model)
                ...
                if current is None:
                    ...
                    if self.monitor_op(current, self.best):
                        ...
                        if self.save_weights_only:
                            self.model2.save_weights(filepath, overwrite=True)
                        else:
                            self.model2.save(filepath, overwrite=True)
            else:
                if self.verbose > 0:
    ...
    def _make_mv_model(self, filepath):
        ...
        for w2, w in zip(self.model2.trainable_weights, self.model.trainable_weights):
